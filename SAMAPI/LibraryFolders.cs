...
namespace Indieteur.SAMAPI
{
    static class LibraryFoldersHelper
    {
        ...
        {
            //Initialize our list of library folders with a count of 1 as we know that the InstallDir is a library folder.
            var libraryFolders = new List<string>(1)
            {
                mainSteamInstallPath
            }; 

            //Let us now locate the file that contains the list of all the steam library folders in the machine.
            string libFileFullPath = mainSteamInstallPath + "\\" + SteamAppsManager.SteamAppsDirname + "\\" + LIBRARY_FOLDERS_NAME; //The full path of the library folders file.

            if (!File.Exists(libFileFullPath))
                return libraryFolders; 

            var vdfReader = new VdfData();
            vdfReader.LoadDataFromFile(libFileFullPath);

            var vNode = vdfReader.Nodes.FindNode(LIBFILE_NODE_NAME); //Find the node that contains the list of steam libraries.

            if (vNode?.Keys == null || vNode.Keys.Count == 0) //If it isn't found or the Nodes key is null or empty, there's nothing else to be done. Return the list of libraryfolders that we already have. (which is just MainSteamInstallPath)
                return libraryFolders;

            foreach (var vKey in vNode.Keys) //List all the keys inside the vNode node.
            {
        ...
        public static List<SteamApp> RetrieveAppsUnderLibraryFolder(string libPath)
        {
            ...
            string steamAppsFolderPath = libPath + "\\" + SteamAppsManager.SteamAppsDirname; //This is the folder that contains the application manifests.
            ...
            {
                bool parseResult = VdfData.TryParseFile(manifest, out VdfData parsedData); //Added this lines of code as there might be App manifests that are corrupted or empty.
